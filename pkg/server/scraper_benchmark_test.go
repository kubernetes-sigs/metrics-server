...
func benchmarkScraper(b *testing.B, g *generator) {
	manageNodeScrape := scraper.NewScraper(&g.client, g.scrapeTimeout, g.metricResolution, g.store)
	g.nodeInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
	...
	server := NewServer(g.nodeInformer, g.podInformer, g.apiserver, g.store, g.metricResolution)
