...
var _ = Describe("Node storage", func() {
	nodeMetric := make(map[string]NodeMetricsPoint)
	It("provides node metrics from stored batches", func() {
		s := NewStorage(60 * time.Second)
		nodeStart := time.Now()

		By("storing first batch with node1 metrics")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(10*time.Second), 10*CoreSecond, 2*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))
		...
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(20*time.Second), 20*CoreSecond, 3*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))
		...
		s.Store(nodeMetricBatch(make(map[string]NodeMetricsPoint)))
	...
	It("handle repeated node metric point", func() {
		...
		By("storing first batch with node1 metrics")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(10*time.Second), 10*CoreSecond, 2*MiByte)}
		batch := nodeMetricBatch(nodeMetric)
		s.Store(batch)
	...
	It("exposes correct node metrics", func() {
		...
		By("storing second batch with node1 metrics")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(20*time.Second), 21*CoreSecond, 3*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))
	...
	It("should detect node restart and skip metric", func() {
		...
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart.Add(15*time.Second), nodeStart.Add(20*time.Second), 5*CoreSecond, 3*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))
	...
	It("should return empty node metrics if decreased data point reported", func() {
		...
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(15*time.Second), 50*CoreSecond, 3*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))

		By("storing CPU usage decreased last metrics")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(25*time.Second), 10*CoreSecond, 5*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))
	...
	It("should handle metrics older then prev", func() {
		...
		By("storing previous metrics")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(15*time.Second), 10*CoreSecond, 3*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))

		By("storing last metrics")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(35*time.Second), 50*CoreSecond, 5*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))

		By("Storing new metrics older then previous")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(5*time.Second), 6*CoreSecond, 2*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))
	...
	It("should handle metrics prev.ts < newNode.ts < last.ts", func() {
		...
		By("storing previous metrics")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(15*time.Second), 10*CoreSecond, 1*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))

		By("storing last metrics")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(35*time.Second), 50*CoreSecond, 4*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))

		By("Storing new metrics prev.ts < node.ts < last.ts")
		nodeMetric["node1"] = NodeMetricsPoint{"node1", newMetricsPoint(nodeStart, nodeStart.Add(25*time.Second), 35*CoreSecond, 2*MiByte)}
		s.Store(nodeMetricBatch(nodeMetric))
...
func nodeMetricBatch(nodes map[string]NodeMetricsPoint) *MetricsBatch {
	return &MetricsBatch{
		Nodes: nodes,
	}
}
