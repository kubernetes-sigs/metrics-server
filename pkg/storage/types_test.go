...
func Test_resourceUsage(t *testing.T) {
	...
	tcs := []struct {
	...
	}{
		{
			...
			last: newMetricsPoint(start, start.Add(20*time.Millisecond), 500, 600, 700),
			prev: newMetricsPoint(start, start.Add(10*time.Millisecond), 300, 400, 500),
			wantResourceList: v1.ResourceList{
				v1.ResourceCPU:    uint64Quantity(uint64(20000), resource.DecimalSI, -9),
				v1.ResourceMemory: uint64Quantity(600, resource.BinarySI, 0),
				"swap":            uint64Quantity(700, resource.BinarySI, 0),
			},
		...
		},
		{
			...
			last:             newMetricsPoint(start, start.Add(20*time.Millisecond), 500, 600, 700),
			prev:             newMetricsPoint(start.Add(20*time.Millisecond), start.Add(10*time.Millisecond), 300, 400, 600),
			wantResourceList: v1.ResourceList{},
			wantTimeInfo:     api.TimeInfo{},
			wantErr:          true,
		},
		{
			...
			last:             newMetricsPoint(start, start.Add(20*time.Millisecond), 100, 600, 700),
			prev:             newMetricsPoint(start, start.Add(10*time.Millisecond), 300, 400, 500),
			wantResourceList: v1.ResourceList{},
			wantTimeInfo:     api.TimeInfo{},
			wantErr:          true,
