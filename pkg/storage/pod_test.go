...
var _ = Describe("Pod storage", func() {
	podMetric := make(map[types.NamespacedName]PodMetricsPoint)
	containerMetric := make(map[string]ContainerMetricsPoint)
	It("provides pod metrics from stored batches", func() {
		...
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(120*time.Second), 1*CoreSecond, 4*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
		...
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(125*time.Second), 6*CoreSecond, 5*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
		...
		s.Store(podMetricsBatch(make(map[types.NamespacedName]PodMetricsPoint)))
	...
	It("returns timestamp of earliest container of pod", func() {
		...
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(110*time.Second), 1*CoreSecond, 4*MiByte)}
		containerMetric["container2"] = ContainerMetricsPoint{"container2", newMetricsPoint(containerStart, containerStart.Add(115*time.Second), 2*CoreSecond, 5*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))

		By("store second batch")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(120*time.Second), 6*CoreSecond, 6*MiByte)}
		containerMetric["container2"] = ContainerMetricsPoint{"container2", newMetricsPoint(containerStart, containerStart.Add(125*time.Second), 7*CoreSecond, 7*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
	...
	It("handle repeated pod metric point", func() {
		...
		By("storing first batch with pod1 metrics")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(110*time.Second), 1*CoreSecond, 4*MiByte)}
		delete(containerMetric, "container2")
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		batch := podMetricsBatch(podMetric)
		s.Store(batch)
	...
	It("should detect container restart and return results based on window from start time", func() {
		...
		By("storing first batch with pod1 metrics")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(10*time.Second), 1*CoreSecond, 4*MiByte)}
		delete(containerMetric, "container2")
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))

		By("storing second batch with pod1 start time after previous batch")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart.Add(15*time.Second), containerStart.Add(25*time.Second), 5*CoreSecond, 5*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
	...
	It("should return pod empty metrics if decreased data point reported", func() {
		...
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(110*time.Second), 20*CoreSecond, 4*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))

		By("storing CPU usage decreased last metrics")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(120*time.Second), 10*CoreSecond, 4*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
	...
	It("should handle pod metrics older then prev", func() {
		...
		By("storing previous metrics")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(1115*time.Second), 20*CoreSecond, 4*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))

		By("storing last metrics")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(1125*time.Second), 40*CoreSecond, 4*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))

		By("Storing new metrics older then previous")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(115*time.Second), 10*CoreSecond, 4*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
	...
	It("should handle pod metrics prev.ts < newNode.ts < last.ts", func() {
		...
		By("storing last metrics")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(125*time.Second), 50*CoreSecond, 5*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))

		By("Storing new metrics prev.ts < node.ts < last.ts")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(120*time.Second), 35*CoreSecond, 5*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
	...
	It("should use start time to return metric in one cycle for fresh new container", func() {
		...
		By("storing first batch with pod1 metrics")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(10*time.Second), 10*CoreSecond, 4*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
		Expect(s.Ready()).To(BeTrue())
	...
	It("should get empty metrics in one cycle for fresh new container's time duration less than 10s between start time and timestamp", func() {
		...
		By("storing first batch with pod1 metrics")
		containerMetric["container1"] = ContainerMetricsPoint{"container1", newMetricsPoint(containerStart, containerStart.Add(9*time.Second), 10*CoreSecond, 4*MiByte)}
		podMetric[podRef] = podMetrics(podRef, containerMetric)
		s.Store(podMetricsBatch(podMetric))
		Expect(s.Ready()).NotTo(BeTrue())
		checkPodResponseEmpty(s, podRef)
...
func podMetricsBatch(pods map[types.NamespacedName]PodMetricsPoint) *MetricsBatch {
	return &MetricsBatch{
		Pods: pods,
	}
}

func podMetrics(pod types.NamespacedName, cs map[string]ContainerMetricsPoint) PodMetricsPoint {
