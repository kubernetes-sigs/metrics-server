...
func TestValidate(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name: "cannot give only --kubelet-certificate-authority, give --kubelet-key-file as well",
			options: &Options{
				KubeletClientCertFile: "a",
			},
			expectedErrorCount: 1,
		...
		{
			name: "cannot give only --kubelet-key-file, give --kubelet-certificate-authority as well",
			options: &Options{
				KubeletClientKeyFile: "a",
			},
			expectedErrorCount: 1,
